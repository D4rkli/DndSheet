...
async function applyCostToCharacter(costStr, sourceName = "Способность") {
  ...
  showBattleToast(`${sourceName}: действие применено`, "success");
    ...
    if (delta.hp) {
      playCombatChipFx(".combat-chip.hp", "is-hit");
      showCombatFloat(".combat-chip.hp", `-${delta.hp}`, "damage");
    }
    if (delta.mana) {
      playCombatChipFx(".combat-chip.mana", "is-mana-flash");
      showCombatFloat(".combat-chip.mana", `-${delta.mana}`, "mana");
    }
    if (delta.energy) {
...
function showCombatFloat(selector, value, kind) {
  ...
  node.className = `combat-float ${kind || "damage"}`;
...
function scheduleCombatAutosave(extra = {}) {
  clearTimeout(combatAutosaveTimer);

  combatAutosaveTimer = setTimeout(async () => {
    try {
      await saveMain(extra);
      setStatus("Боевой HUD сохранён ✅");
    } catch (e) {
      console.error("Combat autosave failed:", e);
      setStatus("Ошибка автосохранения");
    }
  }, 350);
}
...
async function applyRest() {
  ...
  el("f_energy").value = String(newEnergy);
    ...
    if (newHp > hp) {
      playCombatChipFx(".combat-chip.hp", "is-heal-flash");
      showCombatFloat(".combat-chip.hp", `+${newHp - hp}`, "heal");
    }

    if (newMana > mana) {
      playCombatChipFx(".combat-chip.mana", "is-mana-flash");
      showCombatFloat(".combat-chip.mana", `+${newMana - mana}`, "mana");
    }

    if (newEnergy > energy) {
      playCombatChipFx(".combat-chip.energy", "is-energy-flash");
      showCombatFloat(".combat-chip.energy", `+${newEnergy - energy}`, "energy");
    }
...
async function applyMovement() {
  ...
  if (newEnergy < energy) {
    playCombatChipFx(".combat-chip.energy", "is-energy-flash");
    showCombatFloat(".combat-chip.energy", `-${energy - newEnergy}`, "energy");
  }
...
function quickApplyResource(targetId, delta) {
  ...
  input.dispatchEvent(new Event("input", { bubbles: true }));
    ...
    if (delta < 0) {
      playCombatChipFx(".combat-chip.hp", "is-hit");
      showCombatFloat(".combat-chip.hp", String(delta), "damage");
    }
    if (delta > 0) {
      playCombatChipFx(".combat-chip.hp", "is-heal-flash");
      showCombatFloat(".combat-chip.hp", `+${delta}`, "heal");
    }
  }

  if (targetId === "f_mana") {
    playCombatChipFx(".combat-chip.mana", "is-mana-flash");
    showCombatFloat(".combat-chip.mana", delta > 0 ? `+${delta}` : String(delta), "mana");
  }

  if (targetId === "f_energy") {
    playCombatChipFx(".combat-chip.energy", "is-energy-flash");
    showCombatFloat(".combat-chip.energy", delta > 0 ? `+${delta}` : String(delta), "energy");
  }
...
function wireCombatExpandableQuickRows() {
  ...
  const syncExpandedState = () => {
    chips.forEach((chip) => {
      const toggle = chip.querySelector(".combat-chip-toggle");
      if (!toggle) return;
      toggle.setAttribute("aria-expanded", chip.classList.contains("is-expanded") ? "true" : "false");
    });
  };
  ...
  chips.forEach((chip) => {
    chip.addEventListener("click", (e) => {
      const toggleBtn = e.target.closest(".combat-chip-toggle");

      // если нажали на любую другую кнопку внутри карточки — не трогаем раскрытие
      if (e.target.closest("button") && !toggleBtn) return;
...
el("btnFullRestore")?.addEventListener("click", () => {
  const hp = Number(el("f_hp")?.value || 0);
  const mana = Number(el("f_mana")?.value || 0);
  const energy = Number(el("f_energy")?.value || 0);
  ...
  if (hpMax > hp) {
    playCombatChipFx(".combat-chip.hp", "is-heal-flash");
    showCombatFloat(".combat-chip.hp", `+${hpMax - hp}`, "heal");
  }

  if (manaMax > mana) {
    playCombatChipFx(".combat-chip.mana", "is-mana-flash");
    showCombatFloat(".combat-chip.mana", `+${manaMax - mana}`, "mana");
  }

  if (energyMax > energy) {
    playCombatChipFx(".combat-chip.energy", "is-energy-flash");
    showCombatFloat(".combat-chip.energy", `+${energyMax - energy}`, "energy");
  }
